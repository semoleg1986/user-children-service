...
def test_self_create_user_and_children(client: TestClient, jwks) -> None:
    user_id = str(uuid4())
    kid = jwks["keys"][0]["kid"]
    ...
    archive_resp = client.delete(
        f"/v1/user/users/{user_id}/children/{child_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    ...
    restore_resp = client.post(
        f"/v1/user/users/{user_id}/children/{child_id}/restore",
        headers={"Authorization": f"Bearer {token}"},
    )
